...
import { insertEasyvista, fetchAccounts, fetchEasyvistaTypes, fetchCompaniesByExcelCompanyIds, fetchCompanyAdditionalExcelData } from '@/integrations/supabase/utils';
...
const EasyvistaCreateForm: React.FC<EasyvistaCreateFormProps> = ({
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.files && event.target.files[0]) {
      ...
      form.setValue('Anexos', event.target.files[0].name);
    } else {
      setAttachmentFile(null);
      form.setValue('Anexos', null);
    }
  };
  ...
  const onSubmit = async (values: FormData) => {
    ...
    try {
      if (attachmentFile && userId) {
        attachmentUrl = await uploadAttachment(attachmentFile, userId, companyExcelId);
      }
      ...
      const newEasyvista: Omit<Easyvista, 'id' | 'created_at' | 'Ultima actualização' | 'Data Criação'> = {
        ...
        "Anexos": attachmentUrl ? [attachmentUrl] : null,
        ...values,
      };
  ...
  const companyDisplayName = additionalCompanyDetails?.["Nome Comercial"]?.trim() || companyDetails?.Commercial_Name?.trim() || companyDetails?.Company_Name || companyExcelId;

  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 p-1 md:p-4">
        <div className="space-y-1">
          <h2 className="text-2xl font-bold tracking-tight">Criar Novo Registo Easyvista</h2>
          <p className="text-muted-foreground">
            Para a empresa <span className="font-semibold text-primary">{companyDisplayName}</span> (ID: <span className="font-semibold">{companyExcelId}</span>)
          </p>
        ...
        <div className="flex justify-end space-x-2 pt-4">
          ...
          <Button type="submit" disabled={isSubmitting || !userId || !companyDbId || !form.formState.isValid}>
            {isSubmitting ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> A Criar...</> : "Criar Registo"}
          </Button>
        </div>
      </form>
    </Form>
