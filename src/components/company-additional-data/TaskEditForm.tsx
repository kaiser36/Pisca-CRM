...
import { updateTask, fetchEmployeesByCompanyExcelId } from '@/integrations/supabase/utils';
...
const TaskEditForm: React.FC<TaskEditFormProps> = ({ task, onSave, onCancel }) => {
  ...
  useEffect(() => {
    const loadEmployees = async () => {
      ...
      try {
        const fetchedEmployees = await fetchEmployeesByCompanyExcelId(userId, task.company_excel_id);
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 p-4">
        <p className="text-sm text-muted-foreground">
          A editar tarefa para a empresa com ID Excel: <span className="font-semibold">{task.company_excel_id}</span>
        </p>
