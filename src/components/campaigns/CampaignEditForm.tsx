...
const CampaignEditForm: React.FC<CampaignEditFormProps> = ({ campaign, onSave, onCancel }) => {
  ...
  const form = useForm<FormData>({
    ...
    defaultValues: {
      ...
      category: campaign.category || '', // NEW: Default to existing category
    },
  });
  ...
  const onSubmit = async (values: FormData) => {
    ...
    try {
      const updatedCampaign: Partial<Omit<Campaign, 'id' | 'created_at' | 'updated_at' | 'user_id'>> = {
        name: values.name,
      ...
      showError(error.message || "Falha ao atualizar a campanha.");
    } finally {
