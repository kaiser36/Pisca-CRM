...
import { updateProduct } from '@/integrations/supabase/utils';
...
const ProductEditForm: React.FC<ProductEditFormProps> = ({ product, onSave, onCancel }) => {
  ...
  const form = useForm<FormData>({
    ...
    defaultValues: {
      ...
      unidade: product.unidade || 0, // Default to 0 for number type
      preco_unitario: product.preco_unitario || 0,
      preco_total: product.preco_total || 0,
    },
  });
  ...
  const onSubmit = async (values: FormData) => {
    ...
    try {
      const updatedProduct: Partial<Omit<Product, 'id' | 'created_at' | 'updated_at' | 'user_id'>> = {
      ...
      showError(error.message || "Falha ao atualizar o produto.");
    } finally {
